...
{
    public partial class SizingInward : Form
    {
        public SizingInward()
        {
            InitializeComponent();
        ...
        public DataTable FillGrid(int QueryNo)
        {
            try
            {
                ...
                if ((dtReturn != null && dtReturn.Rows.Count > 0))
                {
                    ...
                    {
                       txtPart.Text= lblPart.Text = objRow["Q_Part"].ToString();
        ...
        public DataSet SaveData(int QueryNo, ref string strReturnMSG, ref string strReturnNo, ref Int32 intReturnNo)//string strReturn    DataSet
        {
            try
            {
                ...
                if (QueryNo == 1 || QueryNo == 2)
                {
                    ...
                    hash.Add("@inttotalBeams", Convert.ToInt32(dgvDesign.Rows.Count));
                    ...
                    hash.Add("@decQ_Part",Convert.ToDecimal(lblPart.Text));
                    ...
                    u = Math.Round(Convert.ToDecimal(lblTara.Text) * Convert.ToDecimal(lblPart.Text),3);
  ...
  // as decimal(18,2)=0.00,
        ...
        public void GridCreation()
        {
            try
            {
                ...
                dt.Columns.Add("BINO", typeof(string));            //1
                dt.Columns.Add("CUT", typeof(decimal)); //2
                dt.Columns.Add("BORDER", typeof(string));//3
                dt.Columns.Add("SAT NO", typeof(string));//4
                dt.Columns.Add("BI CODE", typeof(string));//5
                dt.Columns.Add("METER", typeof(decimal));//6
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                d = Math.Round(d + Convert.ToDecimal(dgvDesign.Rows[i].Cells[2].Value.ToString()), 2);
            }
            if (Convert.ToDecimal(txtTotalCuts.Text) == d)
            {
                if (newOrEdit == 0)
                {
            ...
            {
                messageBox frm = new messageBox();
                frm.messageTxt = "Please check cuts that you enterd";
                frm.type = "error";
                frm.ShowDialog();
            }
        }

        private void SizingInward_Load(object sender, EventArgs e)
        {
            FillGrid(301);
            FillGrid(302);
            FillGrid(507);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dgvDesign.Rows.Count; i++)
                {
                    if (dgvDesign.Rows[i].Cells[1].Value.ToString() == cmbBNo.Text)
                    {
                        repete = 1;
                    }
                }
                ...
                if (repete == 0)
                {
                    if (cmbBNo.SelectedIndex != -1)
                    {
                        if (txtCut.Text != "")
                        {
                            if (txtSatNo.Text != "")
                            {
                                ...
                                dr2["X"] = "X";
                                dr2["BINO"] = cmbBNo.Text; //txtBNo.Text;
                                dr2["CUT"] = Convert.ToDecimal(txtCut.Text);
                                dr2["BORDER"] = txtBorder.Text;
                                dr2["SAT NO"] = txtSatNo.Text;
                                dr2["BI CODE"] = txtChallanNo.Text + txtSatNo.Text + cmbBNo.Text;//txtBNo.Text;
                            ...
                            {
                                messageBox frm = new messageBox();
                                frm.messageTxt = "Please enter sat no";
                                frm.type = "error";
                                frm.ShowDialog();
                                txtSatNo.Focus();
                            }
                        ...
                        {
                            messageBox frm = new messageBox();
                            frm.messageTxt = "Please enter beam cut";
                            frm.type = "error";
                            frm.ShowDialog();
                            txtCut.Focus();
                        }
                    ...
                    {
                        messageBox frm = new messageBox();
                        frm.messageTxt = "Please enter beam no";
                        frm.type = "error";
                        frm.ShowDialog();
                        cmbBNo.Focus();
                        //txtBNo.Focus();
                ...
                {
                    messageBox frm = new messageBox();
                    frm.messageTxt = "This Beam No is already added";
                    frm.type = "error";
                    frm.ShowDialog();
                    //txtBNo.Focus();
                    cmbBNo.Focus();
            ...
            finally
            {
               ...
               // txtBNo.Focus();
                ...
                for (int i = 0; i < dgvDesign.Rows.Count; i++)
                {
                    txtTotalMtrs.Text = Math.Round(Convert.ToDecimal(txtTotalMtrs.Text) + Convert.ToDecimal(dgvDesign.Rows[i].Cells[6].Value.ToString()), 2).ToString();
                }
